...
import { collectMessagesForDay } from './whatsapp-scraper.mjs';
...
export async function runPipeline(config, logger) {
  const messages = await collectMessagesForDay({
    date: config.date,
    fromFixture: config.fromFixture,
    logger,
  });

  const imageMessages = messages.filter((m) => m.kind === 'image');
  logger.info('Fotos detectadas', { total: imageMessages.length });
  ...
  logger.info('Pipeline finalizado', { reports, total: rows.length });
  return { rows, reports };
}
