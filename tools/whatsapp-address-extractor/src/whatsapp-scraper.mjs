...
export async function collectMessagesForDay({ date, fromFixture, logger, mode, debugDir, profileDir, waitForEnter }) {
  ...
  const timeline = await scrapeTimeline(page, date, logger);
  const imageCount = timeline.filter((m) => m.kind === 'image').length;

  if (timeline.length === 0 || imageCount === 0) {
...
async function scrapeTimeline(page, targetDate, logger) {
  const raw = await page.evaluate(async ({ targetDate }) => {
    ...
    for (let idx = 0; idx < nodes.length; idx += 1) {
      ...
      const kind = img || hasMediaByAttr || hasMediaButtonNearby ? 'image' : 'text';
      ...
      if (targetDate && datePart !== targetDate) {
        continue;
      }
